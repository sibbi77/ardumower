...
void MowerPerimeter::setup(){
  Console.println(F("MowerPerimeter::setup"));
  // perimeter
...
void MowerPerimeter::setPins(byte idx0Pin, byte idx1Pin){
  idxPin[0] = idx0Pin;
  idxPin[1] = idx1Pin;  
...
int MowerPerimeter::getSmoothMagnitude(char coilIdx){  
  return smoothMag[coilIdx];
}

void MowerPerimeter::printADCMinMax(int8_t *samples){
...
void MowerPerimeter::matchedFilter(char coilIdx){
  int16_t sampleCount = ADCMan.getCaptureSize(idxPin[0]);
  int8_t *samples = ADCMan.getCapture(idxPin[coilIdx]);    
  if (callCounter == 100) {
    // statistics only
    callCounter = 0;
    signalMin[coilIdx] = 9999;
    signalMax[coilIdx] = -9999;
    signalAvg[coilIdx] = 0;  
    for (int i=0; i < sampleCount; i++){
      int8_t v = samples[i];
      signalAvg[coilIdx] += v;
      signalMin[coilIdx] = min(signalMin[coilIdx], v);
      signalMax[coilIdx] = max(signalMax[coilIdx], v);
    }
    signalAvg[coilIdx] = ((double)signalAvg[coilIdx]) / ((double)(sampleCount));
  }
  ...
  mag[coilIdx] = corrFilter(sigcode, subSample, sigcode_size, samples, sampleCount-sigcode_size*subSample, filterQuality[coilIdx]);
  if (swapCoilPolarity) mag[coilIdx] *= -1;        
  // smoothed magnitude used for signal-off detection
  smoothMag[coilIdx] = 0.99 * smoothMag[coilIdx] + 0.01 * ((float)abs(mag[coilIdx]));

  // perimeter inside/outside detection
  if (mag[coilIdx] > 0){
    signalCounter[coilIdx] = min(signalCounter[coilIdx]+1, 3);    
  } else {
    signalCounter[coilIdx] = max(signalCounter[coilIdx]-1, -3);    
  }
  if (signalCounter[coilIdx] < 0){
    lastInsideTime[coilIdx] = millis();
  } 
    
  ADCMan.restart(idxPin[coilIdx]);    
  if (coilIdx == 0) callCounter++;
}

int MowerPerimeter::getSignalMin(char coilIdx){
  return signalMin[coilIdx];
}

int MowerPerimeter::getSignalMax(char coilIdx){
  return signalMax[coilIdx];
}

int MowerPerimeter::getSignalAvg(char coilIdx){
  return signalAvg[coilIdx];
}


float MowerPerimeter::getFilterQuality(char coilIdx){
...
bool MowerPerimeter::signalTimedOut(char coilIdx){
...
int16_t MowerPerimeter::corrFilter(int8_t *H, int8_t subsample, int16_t M, int8_t *ip, int16_t nPts, float &quality){  
